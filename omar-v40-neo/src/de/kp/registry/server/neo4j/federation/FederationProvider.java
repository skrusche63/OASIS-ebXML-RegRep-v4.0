...
public class FederationProvider {
	...
	public FederationProvider(QueryRequestContext request) {
		this.request = request;
	}
	
	public List<RegistryObjectType> getFederates() {
    	...
    	String federation = this.request.getFederation();
		QueryRequest remoteRequest = createQueryRequest(federation);
		...
		try {
			...
			remoteConnection.setRegistryUrl(CanonicalConstants.FEDERATION_REGISTRY_URL);
			remoteConnection.setCredentialInfo(credentialInfo);

			// invoke client side query manager to retrieve the
			// respective registry types
	...
	private QueryRequest createQueryRequest(String federation) {
		...
		query.setQueryDefinition(CanonicalConstants.QUERY_GetFederates);

		// create query parameter
		SlotType queryParam = ebRIMFactory.createSlotType();
		queryParam.setName(CanonicalConstants.QUERY_PARAM_FEDERATION_ID);
		
		StringValueType queryParamValue = ebRIMFactory.createStringValueType();
		queryParamValue.setValue(federation);
